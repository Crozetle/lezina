 ...
 */
public class Motorcycle implements Transport, Cloneable { // [PROTOTYPE] добавлен Cloneable

    // ── Внутренний класс Model (узел списка) ─────────────────────────────
    //
    // [PROTOTYPE] изменено: было "private class Model", стало "static class Model"
    // Причина та же, что и в Car: нестатический внутренний класс хранит
    // скрытую ссылку на внешний Motorcycle, что мешает корректному клонированию.
    private static class Model { // [PROTOTYPE] было: private class Model
        String name  = null;
    ...
    //
    // Клонировать связный список сложнее, чем массив: нельзя просто скопировать
    // ссылку head — оба объекта окажутся с одним общим списком узлов.
    //
    // Алгоритм:
    //   1. super.clone() — поверхностная копия. clone.head == this.head (плохо!).
    //   2. Создаём новую голову (head) для клона — пустой циклический список.
    //   3. Обходим оригинальный список и для каждого узла создаём новый узел
    //      с теми же данными, добавляем его в новый список клона.
    @Override
    public Object clone() throws CloneNotSupportedException {
        // Шаг 1: поверхностная копия (brand, size скопируются автоматически)
        Motorcycle clone = (Motorcycle) super.clone();

        // Шаг 2: новая голова для клона — иначе оба объекта делят один список
        clone.head      = new Model();
        clone.head.prev = clone.head;
        clone.head.next = clone.head;
        clone.size      = 0;

        // Шаг 3: обходим оригинал, создаём новые независимые узлы для клона
        for (Model cur = this.head.next; cur != this.head; cur = cur.next) {
            Model newNode = new Model();
            newNode.name  = cur.name;   // String неизменяем — просто копируем ссылку
            newNode.price = cur.price;  // double — примитив, копируется по значению
